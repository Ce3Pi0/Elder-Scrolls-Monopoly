import { Serializable } from "../abstract/serializable";

export class Timer extends Serializable {
  readonly VALID_GAME_DURATIONS: number[] = [45];

  private duration: number;
  private startTime: number;
  constructor(duration: number) {
    super();

    if (!this.VALID_GAME_DURATIONS.includes(duration))
      throw new Error("Invalid game duration");
    this.duration = duration;
    this.startTime = new Date().getMinutes();
  }

  public serialize(): void {
    const timerData = {
      duration: this.duration,
      startTime: this.startTime,
    };
    localStorage.setItem("timer", JSON.stringify(timerData));
  }
  public deserialize(): Timer | undefined {
    const retrievedData: string | null = localStorage.getItem("timer");
    if (!retrievedData) throw new Error("No timer data found");
    const timerData = JSON.parse(retrievedData);
    this.duration = timerData.duration;
    this.startTime = timerData.startTime;

    return this;
  }
  public expired(): boolean {
    let curTime: number = new Date().getMinutes();
    return Math.abs(curTime - this.startTime) >= this.duration;
  }
}
